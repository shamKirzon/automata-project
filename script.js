...
function computeDivision(input1, input2) {
  function validate(val, label) {
    ...
    if (/[^0-9]/.test(val)) return `INVALID: ${label} must be a positive integer (no letters, decimals, or signs)`;
    if (val.startsWith("0")) return `INVALID: ${label} cannot be zero or have leading zeros`;
    return null;
  }
...
function computeCollatz(input) {
  if (!input || input.trim() === "" || input.includes(" ") || /[^0-9]/.test(input)) {
...
function computeFibonacci(input) {
  function validate(val) {
    ...
    if (val.startsWith("0")) return "INVALID: Input cannot be zero or have leading zeros";
